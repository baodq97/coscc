...
`0003 spec.md` R12 and R21 are the reason this file is longer than the module deserves:
...
`0011` moved it from a JSON file under the working folder into the app's SQLite database.
...
class AHandEditedStoreCannotWidenTheBoundary(unittest.TestCase):
    ...
    def test_an_entry_naming_an_outside_path_is_dropped(self):
        """`0003 spec.md` R21, at the store layer.
...
class ListOperations(unittest.TestCase):
    ...
    def test_remove_does_not_touch_the_directory(self):
        # `0003 spec.md` R18 and C6: the app has no undo and a clone may hold uncommitted work.
        with tempfile.TemporaryDirectory() as d:
            store = Store(d, d)
...
class TheOneShotImportFromJson(unittest.TestCase):
    """`0011 spec.md` R8. A machine set up before this unit keeps its workspaces."""
    ...
    def test_no_file_means_no_migration_row_at_all(self):
        """The common case after `0011` must not pay for the migration."""
        with tempfile.TemporaryDirectory() as d:
            store = Store(d, d)
            store.add("fresh")
            with store.data.connect() as conn:
...
class TheTransactionIsAcrossProcesses(unittest.TestCase):
    """`0005` measured the old arrangement losing 12 of 20 entries.
    ...
    The full claim is `scripts/verify_0005.py` with four writers and twenty entries. What
    ...
    def test_waiting_for_a_held_database_ends_in_an_error_not_a_hang(self):
        """`0011 spec.md` R10. The deadline turns a hang into something sayable."""
        ...
        # `0005 spec.md` C7: a timeout reads like a broken app unless it says what is
