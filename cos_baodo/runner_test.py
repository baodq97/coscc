...
is `spec.md` R9 and the reason `0007` can still be checked after this unit lands.

Nothing here creates a session. What the guards do before a process is spawned is exactly
what is worth testing cheaply; a real run belongs to `scripts/verify_0008.py`.
"""
...
from cos_baodo import policy, runner
...
class ProseStagesCarryNothing(unittest.TestCase):
    ...
    def test_a_prose_stage_that_somehow_gained_tools_refuses_to_run(self):
        ...
        A prose stage with tools would stop being covered by `0007` without anything
