...
def plan_of(data_dir: str | os.PathLike[str] | None, mode: str, journal: Any = None) -> dict[str, Any]:
    ...
    reading and spending nothing beyond the files, the run log and git. `store` is
    `read_store`'s, so it refuses what that refuses.
    ...
    with _Lock(directory):
        ...
        for i, batch in enumerate(parts, 1):
            ...
            row = {
                ...
                "dropped": [] if reason else (rebuilt if i == 1 else []) + dropped + admitted,
