#!/usr/bin/env python3
"""Proof for .cos/0002_no-session-management.
...
def _make_second_project(root: Path) -> Path:
    ...
    """
    project = root / "cos-0002-proof"
    project.mkdir(parents=True, exist_ok=True)
    (project / "README.md").write_text("Temporary project for verify_0002.py\n")
    return project


async def run() -> int:
    f = Failures()
    tmp = Path(tempfile.mkdtemp(prefix="cos0002-"))
