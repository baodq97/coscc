#!/usr/bin/env python3
"""Proof for .cos/0002_no-workspace-management.
...
def claim_2() -> Claim:
    ...
    root = Path(tempfile.mkdtemp(prefix="cos0002-gate-"))
    try:
        (root / "real").mkdir()
...
async def claim_3() -> Claim:
    ...
    root = Path(tempfile.mkdtemp(prefix="cos0002-ws-"))
...
async def run() -> int:
    ...
    print()
    if all(ok):
        print(f"PASS — all three claims of 0002 hold ({WORKSPACES} workspaces).")
        return 0
