...
# --- the craft floor (`0008` R22-R25) ----------------------------------------
#
# These four are the only measurable part of "the page should be good". They are
# necessary, not sufficient, and `0008` spec.md C7 says so in as many words: a page can
# pass every one of them and still be unpleasant. They live here rather than in
# `verify_0008.py` because each one needs a real browser — a layout that overflows and a
# colour that fails contrast are both invisible to an HTTP check.

# Phone, tablet, laptop. `0008` R24 fixes these three.
WIDTHS = (390, 768, 1280)
