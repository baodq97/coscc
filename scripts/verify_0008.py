#!/usr/bin/env python3
"""Proof for .cos/0008_hand-driven-invisible-loop.

Seven propositions, listed in `plan.md ## Proof`. Four of them (8–11) are not here: they
need a real browser and they live in `scripts/verify_0004.py`, beside the other claim that
can only be seen on a rendered page.

Exit codes follow `scripts/verify_0004.py:49`:
...
Two of those codes carry the same weight here as they do in `0004`: a `pr` step that cannot
...
# Only these two carry tools. The other six are the ones `0007` is about.
AUTONOMOUS = ("impl", "pr")

# A second unit, used once, to run a step into its ceiling on purpose.
CEILING_UNIT = "0002_a-step-that-runs-out-of-room"
...
def claim_1() -> Claim:
    ...
    root = Path(tempfile.mkdtemp(prefix="cos0008-gate-"))
    try:
        directory = root / ".cos" / UNIT
        directory.mkdir(parents=True)

        # Before anything is written, the gate has to be capable of saying no. `0004`
...
async def _run_step(http, cwd: str, unit: str, stage: str, mode: str) -> dict:
    ...
    into the runner, for the same reason `0002` gave: a path nobody walks proves nothing.
    """
...
async def _init_tools(config, cwd: str) -> tuple[list[str], list[str]]:
    ...
    agrees with itself; `0007` measured that the table is not the whole answer.
    """
...
async def claim_5(config, cwd: str) -> Claim:
    ...
    if servers:
        c.note(
            ...
            "That is `0007` — accepted plan, no implementation. `--tools` names the "
            "built-in set only, so it cannot subtract these. The grant is empty; the "
            "session is not, and this claim stays red until `0007` lands."
        )
    return c
