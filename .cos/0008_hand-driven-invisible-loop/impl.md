...
**11. `verify_0008.py` và tài liệu** (`029926b`). Bảy mệnh đề, mã thoát theo
`scripts/verify_0004.py:49`. `.claude/harness.md` và `.claude/CLAUDE.md` sửa cho khớp.
...
`verify_0008.py` hỏi **0 tool và 0 MCP server**, vì đo riêng danh sách tool cho kết quả
...
uv run python scripts/verify_0002.py  exit 0 — cả 5 claim, 2 project
uv run python scripts/verify_0003.py  exit 0 — cả 3 claim
uv run python scripts/verify_0004.py  exit 0 — cả 5 check, kể cả sàn thẩm mỹ
uv run python scripts/verify_0005.py  exit 0 — 20/20 entry, pull bị chặn đúng lúc
uv run python scripts/verify_0008.py  exit 1 — xem bên dưới
```

`verify_0008.py`, chạy không đặt `COS_PROOF_REPO`:
...
- **Mệnh đề 5 KHÔNG đạt** — session giai đoạn chữ, grant rỗng, vẫn có `microsoft-learn` và
  ...
  `0007`, unit có plan accepted và chưa một dòng code.
...
số tool thì `0008` đã tự cấp cho mình một dấu xanh.
...
2. **`0007` chưa làm, nên mệnh đề "0 tool" chỉ đúng một nửa.** `spec.md` C1 khuyên làm
   `0007` trước `0008`; đã không làm vậy. Giờ `verify_0008.py` là chỗ nêu tên vấn đề đó
   mỗi lần chạy.
...
5. **`0005` C2 rộng thêm.** Journal ghi thường xuyên hơn hẳn danh sách workspace, mà hai
