...
Trang web dựng ở `terminal-only-access` chỉ nói được với **một** session Claude Code đã được mở sẵn trong
...
cái việc mà `terminal-only-access` sinh ra để khỏi phải làm. `terminal-only-access` giải được "với tới session đang chạy";
nó không giải được "làm việc với nhiều project", và đó vẫn là vấn đề gốc.

Đây cũng là chỗ ràng buộc cứng của `terminal-only-access` — không dùng `claude-agent-sdk`
(`terminal-only-access/intent.md:50`) — hết tác dụng. Ràng buộc ấy đúng cho việc
...
Phép đo cố ý **không cần trình duyệt**. Bài học đắt nhất của `terminal-only-access` là phần chứng minh của nó
...
- **`channel/` của `terminal-only-access`:** bị ngừng sử dụng, **không bị xoá**. Xem `## Constraints`.
...
- **Không xoá `channel/`.** `scripts/verify-0001.mjs:9` import `channel/transcript.mjs`; xoá
  là mất lệnh duy nhất chứng minh `terminal-only-access` từng đạt. Đánh dấu retired thì được, gỡ thì không.
...
3. Web app giữ trạng thái gì giữa các lần tải? Nếu nó cũng chỉ là khung nhìn như `terminal-only-access` thì
