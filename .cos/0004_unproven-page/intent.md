...
`0003` đóng với ba mệnh đề xanh và một trang hỏng.

`spec.md` C11 của `0003` (`.cos/0003_no-workspace-management/spec.md:227-233`) đã nói trước:
...
(`.cos/0003_no-workspace-management/plan.md:183-206`). Trang tải, render đủ mọi thành phần,
...
người bảo thử. Trong lúc nó đang hỏng, `scripts/verify_0003.py` vẫn xanh cả ba mệnh đề và
...
`0003` vừa cho thấy chính xác một bộ ba mệnh đề xanh trên một trang chết trông như thế nào.
...
- **Phạm vi hẹp: đúng loại lỗi đã cắn.** Trang tải, nối được, và đọc được dữ liệu sống từ
  ...
  session thật, tức tiêu hạn mức và cần mạng, cho phần mà `scripts/verify_0003.py` đã chứng
  minh ở tầng API rồi.
...
- **Phải đỏ được.** Lệnh phải chứng minh nó đỏ trên cấu hình hỏng đã biết, không chỉ xanh
  trên cấu hình tốt.
- **Không sửa `0003`.** `intent.md` của `0003` đặt "không cần trình duyệt" thành một phần
  kết quả của nó, và kết quả đó **đã đạt**. Unit này không lật điều đó và không viết lại
  artifact nào của `0003`; nó thêm một bằng chứng thứ hai cho một thứ khác.
...
4. Lệnh này có nên chạy cả khi `0003` chưa build frontend không, hay cứ đỏ? Đỏ vì chưa build
   là đỏ đúng, nhưng dễ bị đọc nhầm thành trang hỏng.
5. Một lệnh kiểm cần trình duyệt thì có tự mâu thuẫn với bài học của `0001` — rằng bằng
   chứng cần người ngồi đó thì làm chậm mọi vòng lặp
   (`.cos/0002_no-session-management/intent.md:33-34`)? Ở đây trình duyệt chạy headless nên
