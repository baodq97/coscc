...
có từ trước `0011` được nhập một lần rồi ghi vào bảng `migrations`; **file gốc không bị
xóa**.
...
`cos_baodo/screens.py` là sáu màn của `0009`, mọi giá trị lấy từ state đó.
...
`cos_baodo/studio.py` **không đổi một dòng nào** — đó là phát biểu rõ nhất về thứ mà `0009`
đã chốt: phần nhìn.

**Bằng chứng** — commit `0fac3c4`. `scripts/verify_0004.py` trỏ sang trang mới, giữ nguyên
bốn kiểm tra sàn và negative control. `scripts/verify_0011.py` là mới.
`scripts/verify_0009.py` bị xóa.
...
`verify_0011.py` ban đầu chờ 2,4 giây cho danh sách hội thoại làm mới và báo đỏ. Chẩn đoán
...
- `npm test` — **31** Node + **279** Python test, số lấy từ output runner. Mốc của `0009`
...
- `uv run python scripts/verify_0005.py` — `20 entries survive 4 processes writing at once`.
  Đây là **R9**, và `spec.md` C2 yêu cầu đo lại chứ không kế thừa. Claim 1 còn được chạy
  riêng **5 lần liên tiếp**, cả 5 đều 20/20.
- `uv run python scripts/verify_0004.py` — exit 0, không phải 2. Năm claim xanh, trong đó
  có negative control: cùng phép kiểm **đỏ** khi không có backend phía sau.
- `uv run python scripts/verify_0011.py` — exit 0, `5/5 flows work on real data and survive
...
**Không chạy:** `scripts/verify_0002.py`, `verify_0003.py`, `verify_0008.py`. Hai cái đầu
tốn quota và clone qua mạng; `verify_0008.py` cần `COS_PROOF_REPO` và đẩy một branch. Cả ba
...
- **Ba proof chưa chạy lại** — xem trên. `verify_0003.py` đáng lo nhất trong ba, vì claim 2
  của nó là claim đã bị sửa tay (chuyển từ sửa file JSON sang sửa bảng SQLite).
- **Người khởi xướng chưa duyệt thiết kế.** Họ nói "khá ưng ý" về prototype của `0009` và
...
- **`verify_0004.py` và `verify_0011.py` chồng nhau một phần** (`spec.md` open question 4).
