...
- **A session reads nothing of `~/.claude/`, and of the project only what the app hands it.**
  Since `0088` `sessions._options` sets `setting_sources=[]` and `strict_mcp_config=True` for
  every session. Until then it set `None`, commented "no project/user settings", which on
  claude-agent-sdk 0.2.158 and 0.2.159 passes no flag at all — so the CLI loaded every source,
  and each stage carried the machine's MCP servers, skills, plugins, hooks and
  `permissions.allow`. Four consequences. A proxy or a key in the `env` block of
  `~/.claude/settings.json` is no longer used: it belongs in the service's env file
  (`docs/install.md`). `CLAUDE.md`, `.claude/CLAUDE.md` and every rule without `paths:` reach
  the session through `coscc/instructions.py`, in the system prompt — handed over as a file in
  the session's data root, never as an argument, since Linux refuses one argument past 128 KiB
  with `E2BIG` and every session there would fail to start; a rule **with** `paths:`
  — `coscc-app.md` and `ui-standard.md` here — arrives only as a line telling the session to
  `Read` it, and nothing checks that it did (the `start` row's `instructions` lists what was
  sent, not what was read). `@path` in those files is not resolved, and `CLAUDE.local.md`,
  parent directories and `.claude/settings*.json` of the project are not read. And the
  CLI's own built-in skills and slash commands are still in every init: no option measured
  removes them (`.cos/0088_*/spike.md ## U8`).
