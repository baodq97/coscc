...
| `scripts/measure_context.py` | **spends real quota.** One real `impl`-grant session per `--repeat` (default 1) on `claude-sonnet-5[1m]`, a one-line prompt; roughly 9–14k tokens billed per session, measured `0032_impl-fills-its-context-with-whole-files-and-refusals`, 2026-09-23. Review round 1, F3: a single sample was not stable enough to trust against `--baseline` — five identical runs split 9150/10322 — so a `--repeat N` run (`plan.md ## Proof` asks for 3) costs `N` sessions, not one |
